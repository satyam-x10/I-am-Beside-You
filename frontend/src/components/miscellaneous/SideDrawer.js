  ...
  return (
    <React.Fragment >
      ...
      <Drawer placement="left" onClose={onClose} isOpen={isOpen}>
        <DrawerOverlay />
        <DrawerContent>
          <DrawerHeader bg="#182126" color="white" borderBottomWidth="1px">Search Contacts</DrawerHeader>
          <DrawerBody bg="#319795">
            <Box d="flex" pb={2}>
              <Input bg="#182126" color="white"
                placeholder="Search by name or email"
                mr={2}
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
              <Button bg="#182126 " color=" white" onClick={handleSearch}>Go</Button>
            </Box>
            {loading ? (
              <ChatLoading />
            ) : (
